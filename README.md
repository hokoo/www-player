...
Мини-плеер для браузера для легендарной игры Что Где Когда. Плеер имеет отдельные кнопки воспроизведения для каждого аудиофайла, индивидуальные уровни громкости и перекрытие дорожек (overlay).
...
```
    ...
    Убедитесь, что вы находитесь в каталоге с файлом `run-player.command`, либо укажите полный путь к файлу.
...
- Кнопка «Остановить сервер» в боковой панели завершает работу локального сервера без открытия терминала и пытается закрыть окно браузера.
